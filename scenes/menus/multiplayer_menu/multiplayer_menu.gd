...
@onready var room_code_display_label: Label = %RoomCodeDisplayLabel
@onready var room_code_line_edit: LineEdit = %RoomCodeLineEdit
...
func _ready():
	...
	room_code_line_edit.text_changed.connect(_on_room_code_text_changed)
	room_code_line_edit.text_submitted.connect(_on_room_code_text_submitted)
...
func _on_room_code_text_changed(new_text: String) -> void:
	var saved_caret = room_code_line_edit.caret_column
	
	# Force uppercase
	room_code_line_edit.text = new_text.to_upper()
	
	# Restore cursor position so it doesn't jump to the end
	room_code_line_edit.caret_column = saved_caret

func _on_room_code_text_submitted(_submitted_text: String) -> void:
	_on_join_pressed()
...
func _on_host_pressed():
	var random_code = generate_random_room_code()
	room_code_display_label.text = random_code
	MultiplayerConfig.start_hosting(random_code)
	get_tree().change_scene_to_packed(game_scene)

func _on_join_pressed():
	var code = room_code_line_edit.text
	MultiplayerConfig.start_joining(code)

func generate_random_room_code() -> String:
	var chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789" # Omitted confusing letters like I, O, 1, 0
	var code = ""
	for i in range(4):
